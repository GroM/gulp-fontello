	...
	return through2.obj(function (file, enc, callback) {
		...
		var processResponse = function (zipContents, callback) {
			var
				...
				finalFontDir = opts['font'] ? opts['font'] : 'font';
				;

			zipEntries.forEach(function (zipEntry) {
				var dirName, fileName, pathName, _ref, fileExt, finalFontDir;
				...
				finalFontDir = (dirName ? ((opts[dirName] ? opts[dirName] : dirName) + '/') : '')
				if(fileExt == '.css')
				{
					if(rpl.s)
					{
						content = new Buffer(String(content).replace(rpl.s, rpl.d));
					}
					if(opts['preprocess'])
					{
